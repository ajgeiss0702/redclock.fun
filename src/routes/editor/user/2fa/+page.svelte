    ...
    onMount(() => {
        let interval = setInterval(() => {
            ...
            if(timeTilExpiration >= 0) {
                countdownString = getTimeString(timeTilExpiration);
            } else {
                countdownString = "0s";
                expired = true;
...
<div class="limit mx-auto text-left">
    ...
    <div class="card mx-auto p-4 mb-32">
        {#if data.user.has2fa}
            ...
            <br>
            If you have lost your two-factor authentication method, please <a href="mailto:support@redclock.fun">contact support</a>
        {:else}
            {#if !expired}
